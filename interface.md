...
调用此接口获取配置的链列表及其下的群组列表信息。

#### 接口URL

**http://localhost:5003/WeBASE-Transaction/chain/config**
...
| 3        | 返回数据 | data       | List     |              | 是       |                   |
| 3.1      |          |            | Object   |              | 是       |                   |
| 3.1.1    | 链编号   | chainId    | Int      |              | 是       |                   |
| 3.1.2    | 群组列表 | groupList  | List     |              | 是       | 链下的群组列表    |
...
| 203001 | chainId has not been configured |      | 请求的链编号未配置       |
| 203002 | groupId has not been configured |      | 请求的群组编号未配置     |
| 203003 | web3jMap is empty               |      | web3jMap为空，初始化异常 |
...
| 203301 | node request failed             |      | 节点请求失败             |
| 203302 | transaction failed              |      | 交易失败                 |
| 203303 | request function is error       |      | 请求方法名错误           |